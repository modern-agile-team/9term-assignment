function solution(N, stages) {
    const answer = [];
    const copyedStages = [...stages];
    //실패한 스테이지 개수 확인
    const failedStage = copyedStages.reduce((acc, cur) => { acc[cur] = (acc[cur] || 0 ) + 1; 
        return acc;
    }, {});
    ...
    for (let j = 0; j < failedRate.length; j++) {
        answer.push(failedRate[j].stage);
    }
