function solution(numbers, hand) {
    ...
    const answer = numbers.reduce((acc, cur) => {
        ...
        // 나머지 2 (가운데 열: 2, 5, 8, 0)
            ...
            const getColumn = (num) => Math.floor((num - 1) / 3); // 행 계산 함수
            const getRow = (num) => Math.floor((num - 1) % 3); // 열 계산 함수

            // 현재 숫자에서 왼손, 오른손 위치 간의 맨해튼 거리 계산
            const left = Math.abs( getColumn(cur) - getColumn(handPosition.left) ) + Math.abs( getRow(cur) - getRow(handPosition.left) );
            const right = Math.abs( getColumn(cur) - getColumn(handPosition.right) ) + Math.abs( getRow(cur) - getRow(handPosition.right) );
